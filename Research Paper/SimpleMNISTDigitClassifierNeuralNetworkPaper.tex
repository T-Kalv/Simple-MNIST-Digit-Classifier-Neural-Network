...
Previous research into using the MNIST dataset for classifying and recognising handwritten digits has produced a wide-range of models with a variety of complexity and performance benefits to the traditional manual, human-validation of handwritten digits. LeNet-5 \cite{lenet}, introduced in 1998 having achieved appoximately a 1\% \cite{lenet} test error probability which resulted in appoximately 99\% probability accuracy with the MNSIT dataset, implemented using CNN (Convolutional Neural Networks). While LeNET-5 \cite{lenet}, achieved a respectable, high probability accuracy using CNN, Max Pooling with 2 convolutions, 2 subsampling, 3 fully connected layers and around 60000 trainable parameters, this paper investigates the performance of a simple multilayer perceptron (MLP) using a feed-forward neutal network trained from scratch without the need of heavy, high-level frameworks. However, more modern implementations do use these heavy, high-level frameworks such as PyTorch \cite{PyTorch}, Keras \cite{Keras}, TensorFlow \cite{TensorFlow}, JAX \cite{JAX} utilising the performance benefits of GPUs (Graphics Processing Unit), NPU (Neural Processing Unit), TPUs (Tensor Processing Units) which help to optimise the speed of learning/training as well with the use of advanced optimisers (including but not limited to Adam). Here due to the added cost of utilising such performance enhancers, the system here presented in this paper will instead utilise modern CPUs for computation and training.
...
\begin{figure}[H]
  ...
  \caption{Sample handwritten digits from the MNIST dataset. Image by Suvanjanprasai, CC BY-SA 4.0 (see \cite{mnsitpic})}
...
\vspace{1em}
Firstly, a One-Hot Encoding algorithm is applied (as illustrated in 'Figure 4: One-Hot Encoding Algorithm') where it creates a matrix array of zeros with 10 output classes, 0 to 9, and goes through each row in the matrix specified by the label Y and sets it to 1. Then it tranposes the matrix so that each column is an example instead of a row. It is mathematically defined as:
\begin{figure}[ht]
\centering
$$\mathrm{one\_hot\_Encoding}(i, K) = \begin{bmatrix} 0 \\ 0 \\ \vdots \\ 1 \\ \vdots \\ 0 \end{bmatrix}, \quad \text{where the $1$ appears at index $i$ and $K$ is the total number of classes.}$$
\caption{One-hot encoding vector with a 1 at index $i$ (dimension $K$).}
\label{fig:onehot}
\end{figure}
...
\subsection{Training}
\begin{lstlisting}[language={},mathescape=true, caption={Initialise Parameters Pseudocode}]
...
\begin{lstlisting}[language={},mathescape=true, caption={Update Parameters Pseudocode}]
...
In order to evaluate the performance of the 'Simple MNIST Digit Classifier Neural Network', the MNIST dataset for classifying and recognising handwritten digits was used as the benchmark. This MNIST dataset consists of 60000 training images for training the model and 10000 testing images for testing the trained model. Each image is 28x28 black and white pixel image with a total of 784 pixels per image, labelled with the coresponding digit classifying '0 to 9' digit. This benchmark was selected for this Research Project as it serves as the main benchmark for evaluating image processing systems/ machine learning algorithms such as for this 'Simple MNIST Digit Classifier Neural Network' as well as allowing for direct comparison with the LeNet-5 architecture (LeCun et al., 1998) \cite{lenet}.
...
\bibitem{jayadevan2011survey}
R. Jayadevan, S. Kolhe, P. M. Patil, U. Pal,
“Automatic processing of handwritten bank cheque images: A survey,”
*International Journal on Document Analysis and Recognition*, Dec. 2011.

\bibitem{kaggle-train-digits}
Kaggle, “Digit Recognizer – train.csv dataset.” \\
\url{https://www.kaggle.com/competitions/digit-recognizer/data?select=train.csv}

\bibitem{kaggle-test-digits}
Kaggle, “Digit Recognizer – test.csv dataset.” \\
\url{https://www.kaggle.com/competitions/digit-recognizer/data?select=test.csv}

\bibitem{simplemnsit}
Simple MNIST Digit Classifier Neural Network Source Code - GitHub \\
\url{https://github.com/T-Kalv/Simple-MNIST-Digit-Classifier-Neural-Network}

\bibitem{lenet}
LeNet-5 Research Paper \\
\url{https://axon.cs.byu.edu/~martinez/classes/678/Papers/Convolution_nets.pdf}
...
\bibitem{JAX}
JAX - Website \\
\url{https://docs.jax.dev/en/latest/index.html}

\bibitem{mnsitpic}
Suvanjanprasai, “Image of MNIST Numbers,” Wikimedia Commons, CC BY-SA 4.0.\\
\url{https://commons.wikimedia.org/w/index.php?curid=156115980}


\bibitem{result}
Simple MNIST Digit Classifier Neural Network Juypter Notebook - GitHub\\
\url{https://github.com/T-Kalv/Simple-MNIST-Digit-Classifier-Neural-Network/blob/main/SimpleMNISTDigitClassifier.ipynb}
